...
| `video_management` | ビデオコンテンツの管理 | **経営層** | **[表示]** サイドバーに「コンテンツ管理」表示<br>**[操作]** `/dashboard/contents`の「ビデオ管理」タブでCRUD操作が可能 | **[表示]** 公開されたビデオコンテンツを閲覧できる<br>**[返信]** この権限を持つユーザーは、**自身が投稿者であるビデオ**のコメントに返信可能。 |
| `message_management` | 経営層メッセージの管理 | **経営層** | **[表示]** サイドバーに「コンテンツ管理」表示<br>**[操作]** `/dashboard/contents`の「メッセージ管理」タブでCRUD操作が可能 | **[表示]** 公開された経営層メッセージを閲覧できる<br>**[返信]** この権限を持つユーザーは、**自身が投稿者であるメッセージ**のコメントに返信可能。 |
...
| `can_comment` | コメント投稿の基本権限 | **全ロール** | (管理者画面での直接的な機能はない) | **[投稿]** この権限があれば、コンテンツに対して新しいコメント（トップレベル）を投稿できる。<br>**[返信]** この権限だけでは**返信は不可**。返信には追加の権限が必要。 |
...
*   **コメントへの返信:**
    *   以下の **すべての条件** を満たす必要があります。
        ...
        2.  **かつ、** 以下のいずれかに該当する。
            ...
            *   返信しようとしているコメントスレッドの**最初の親コメントの投稿者**が自分である。

### 6.2. 代理投稿と返信権限のルール

代理投稿機能を使用した場合、返信の権限は「実際に投稿操作を行った代理人」ではなく、「発信者として設定された本人」に紐づきます。
