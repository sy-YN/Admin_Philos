...
1.  **基本情報入力**: `タイトル`、`対象組織`、`単位`を入力します。対象組織は「部署」タイプのみ選択可能です。
...
従業員向けアプリでは、管理者が設定した目標を視覚的なグラフとしてダッシュボードに表示します。

### 4.1. 目標の閲覧権限

*   従業員は、**自身が所属する組織**、およびその組織の**上位階層にあたる全ての親組織**の目標を閲覧できます。
*   例えば、「営業1課」のメンバーは、「営業1課」「関東営業部」「東日本事業本部」といった階層の目標をすべて閲覧対象とすることができます。

### 4.2. 表示目標の選択と固定表示

*   **目標選択UI**:
    *   ダッシュボードの組織目標表示エリアには、表示する目標を切り替えるためのUI（例: ドロップダウンセレクター）を設置します。
    *   セレクターの選択肢には、上記4.1の権限ルールに基づき、閲覧可能なすべての組織（自組織とその親組織）がリストアップされます。
*   **表示ロジック**:
    *   ユーザーがセレクターから特定の組織（例: `SYS関東事業本部`）を選択すると、その組織に紐づく目標のうち、`status: 'active'` に設定されている目標ウィジェットがダッシュボードに表示されます。
    *   この選択はユーザーごとに永続化され、次回以降のアクセス時も、前回選択した組織の目標がデフォルトで表示されます。
*   **【提案】ユーザー設定の保存場所**:
    *   ユーザーの選択を保存するため、`users`コレクションの各ユーザードキュメントに、選択した**組織ID**を保存するフィールド（例: `pinnedOrganizationGoalId: string`）を追加することを推奨します。

### 4.3. グラフの表示ロジック

*   **初期表示**:
    1.  まず、表示対象となった`goals`ドキュメントから`defaultGranularity`と`defaultIsCumulative`の値を読み込み、グラフの初期表示状態を決定します。
    2.  ユーザーはUI上のスイッチやドロップダウンで、この初期表示を自由に変更できます。

*   **「実績を積上」スイッチの状態別 表示ルール**:
    ...
    *   **ツールチップ例**: `週ごと`表示で`8/1-8/7`の棒にカーソルを合わせると → `「8月1日〜8月7日の実績: 80件」`
    ...
    *   **ツールチップ例**: `週ごと`表示で`8/1-8/7`の時点にカーソルを合わせると → `「累計実績: 80件」「累計達成率: 8%」`
