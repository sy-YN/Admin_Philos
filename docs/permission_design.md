...
```
### 3.4. コンテンツのデータモデルへの追加提案
代理投稿機能の実装に伴い、`executiveMessages`や`videos`などのコンテンツドキュメントに、以下のフィールドを追加することを推奨します。

*   **`creatorId` (String):** このコンテンツを実際に作成（操作）したユーザーのUID。
*   **`authorId` (String):** （既存）コンテンツの発信者として表示されるユーザーのUID。

これにより、「誰が（`creatorId`）、誰の代理で（`authorId`）投稿したか」という監査証跡を明確に残すことができます。
...
| `appearance_management` | アプリの外観（ロゴ・色）設定 | **管理者** | **[表示]** サイドバーに「外観設定」表示<br>**[操作]** `/dashboard/appearance`でアプリ名、ロゴ、テーマカラーを編集可能 | **[反映]** 設定されたロゴや色がアプリ全体に反映される |
| `video_management` | ビデオコンテンツの管理 | **経営層** | **[表示]** サイドバーに「コンテンツ管理」表示<br>**[操作]** `/dashboard/contents`の「ビデオ管理」タブでCRUD操作が可能 | **[表示]** 公開されたビデオコンテンツを閲覧できる<br>**[返信]** この権限を持つユーザーは、`can_comment`権限も持っていれば、**自身が投稿者であるビデオ**のコメントに返信可能。 |
| `message_management` | 経営層メッセージの管理 | **経営層** | **[表示]** サイドバーに「コンテンツ管理」表示<br>**[操作]** `/dashboard/contents`の「メッセージ管理」タブでCRUD操作が可能 | **[表示]** 公開された経営層メッセージを閲覧できる<br>**[返信]** この権限を持つユーザーは、`can_comment`権限も持っていれば、**自身が投稿者であるメッセージ**のコメントに返信可能。 |
| `tag_management` | コンテンツタグのマスター管理 | **管理者, 経営層** | **[表示]** コンテンツ管理画面に「タグを管理」ボタン表示<br>**[操作]** タグの追加・編集・削除が可能 | (本機能は従業員向けアプリには存在しない) |
...
| `can_comment` | コメント投稿の基本権限 | **全ロール** | (管理者画面での直接的な機能はない) | **[投稿]** この権限がなければ、いかなるコメント活動（新規投稿・返信）も行えない。<br>**[返信]** この権限だけでは**返信は不可**。返信には追加の権限が必要。 |
...
コメントに関する操作は、複数の権限の組み合わせによって制御されます。

*   **新規コメント投稿:**
    *   `can_comment` 権限を持っている **すべてのユーザー** が可能です。

*   **コメントへの返信:**
    *   以下の **すべての条件** を満たす必要があります。
        1.  `can_comment` 権限を持っている。
        2.  **かつ、** 以下のいずれかに該当する。
            *   `admin` または `executive` の役割（ロール）である。
            *   対象コンテンツの `authorId` と自身の `userId` が一致する（自分が投稿したコンテンツへの返信）。

### 6.2. 代理投稿と返信権限のルール

代理投稿機能を使用した場合、返信の権限は「実際に投稿操作を行った代理人（`creatorId`）」ではなく、「**発信者として設定された本人（`authorId`）**」に紐づきます。

*   **シナリオ例:**
    1.  秘書のAさん（`proxy_post_message`権限あり）が、CEOのBさん（`executive`ロール）の代理としてメッセージを投稿した。
    2.  データベースには `creatorId: "AさんのUID"`, `authorId: "BさんのUID"` と記録される。
    3.  その投稿に一般社員からコメントがついた。
*   **返信可能なユーザー:**
    *   **CEOのBさん:** 自分が投稿者（`authorId`）であるため返信可能。
    *   **秘書のAさん:** 投稿操作を行っただけなので**返信不可**。
    *   **他の `admin` や `executive`:** 全体的な返信権限があるため返信可能。
