...
本機能の核心は、**期間が重複した場合は管理者が設定した表示順（優先度）に従い**、かつ**「期間指定メッセージ」を「日替わりメッセージ」よりも優先して表示する**ロジックと、複数ユーザー環境でも正確に日替わりを実現する**カウンター更新制御**にあります。
...
-   **ドキュメントの主なフィールド**:
    ...
    -   `order` (number): **期間が重複した場合の優先順位。数値が小さいほど優先度が高い。**
    -   `authorId` (string): 作成者のUID
    -   `authorName` (string): 作成者の表示名
...
1.  **【最優先】期間指定メッセージの検索**:
    -   まず、`fixedCalendarMessages`コレクションを`order`の昇順で取得し、その中から**今日の日付が`startDate`と`endDate`の範囲内に含まれる**ドキュメントをすべて検索します。
    -   該当するドキュメントが**見つかった場合**、その中で最も`order`が小さい（＝優先度が高い）ものを1つだけ表示し、以降の処理はすべて中断します。日替わりカウンターの更新も行われません。
    -   `G((処理終了))`
...
3.  **【カウンター更新】日付比較とカウンター更新処理**:
    ...
    -   **日付が変わっていなかった場合**は、既に他の誰かがカウンターを更新済みなので、書き込みは行わず、読み取った`dailyLoop-counter`の値を使ってメッセージを表示するだけです。
