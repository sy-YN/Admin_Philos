...
*   **`composed`**: 実績と目標不足分/超過分を積み上げ棒グラフで、達成率を折れ線グラフで表示。
*   **`bar`**: 実績と目標不足分/超過分のみを積み上げ棒グラフで表示。
*   **`line`**: 実績がある月までを実線、未来の月を点線（目標値）で繋いだ1本の折れ線グラフで表示。

#### `profit_margin` (営業利益率)

*   **`line`**: 各月の営業利益率（`operatingProfit` / `salesRevenue`）を折れ線グラフで表示。
...
*   **`bar`**: 各月の「遵守」「軽微遅延」「遅延」の件数を積み上げ棒グラフで表示（月が横軸、件数が縦軸）。
*   **`pie`**: 対象年度の全月の件数を**合計**し、「遵守」「軽微遅延」「遅延」の全体構成比を単一の円グラフで表示。
